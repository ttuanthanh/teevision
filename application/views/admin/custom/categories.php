...
echo form_open(site_url('admin/custom/categories'), $attribute);
?>
<div class="row">
	<div class="col-md-6">
		<div class="row">
			...
			<div class="col-md-4">
				<?php 
					$searchs = array('name' => 'search_category', 'id' => 'search_category', 'class' => 'form-control', 'placeholder' => lang('custom_search_category_place'), 'value'=>$search);
	...
	<div class="col-md-6">
		<p style="text-align:right;">
			<a class="btn btn-primary tooltips" title="<?php echo lang('add'); ?>" href="<?php echo site_url().'admin/custom/editcategory'; ?>">
			...
			<a class="btn btn-bricky tooltips" title="<?php echo lang('delete'); ?>" id="btn-delete" href="javascript:void(0);"> 
			...
			<tbody>
				...
				?>
					<tr>
						...
						<td><a href="<?php echo site_url().'admin/custom/editcategory/'.$category->id; ?>" ><?php echo $category->title; ?></a></td>
						<td>
							<?php 
						...
						<td style="width: 10%;" class="center"><?php if($category->image != '') echo '<img src="'.base_url($category->image).'" style="height: 70px;"/>'; ?></td>
						<td style="width: 10%;" class="center"><?php if ($category->published == 1) { ?>					   
								<a class="btn btn-success btn-xs tooltips" href="<?php echo site_url().'admin/custom/unpublishcate/'.$category->id; ?>" title="<?php echo lang('click_unpublish');?>"><?php echo lang('publish'); ?></a>
							<?php } else { ?>
								<a class="btn btn-danger btn-xs tooltips" href="<?php echo site_url().'admin/custom/publishcate/'.$category->id; ?>" title="<?php echo lang('click_publish');?>"><?php echo lang('unpublish'); ?></a>
							<?php } ?>
						</td>
						<td style="width: 10%;" class="center">
							<div class="visible-md visible-lg hidden-sm hidden-xs">
								<a class="btn btn-teal tooltips" data-original-title="Edit" data-placement="top" href="<?php echo site_url().'admin/custom/editcategory/'.$category->id; ?>">
									<i class="fa fa-edit"></i>
								</a>
								<a class="btn btn-bricky tooltips" href="<?php echo site_url().'admin/custom/delcategory/'.$category->id; ?>" data-original-title="<?php echo lang('remove');?>" data-placement="top" onclick="return confirm('<?php echo lang('products_categories_confirm_delete_msg');?>');"/>
...
<script type="text/javascript">
   ...
   jQuery('#btn-publish').click(function(){
		...
		{
			jQuery('#fr-categories').attr('action', '<?php echo site_url().'admin/custom/publishcate'?>');
			jQuery('#fr-categories').submit();
		}else
		{
			alert('<?php echo lang('custom_categories_checked_checkbox_msg');?>');
		}
   });
   ...
   jQuery('#btn-unpublish').click(function(){
		if(jQuery('input.checkb').is(':checked'))
		{
			jQuery('#fr-categories').attr('action', '<?php echo site_url().'admin/custom/unpublishcate'?>');
			jQuery('#fr-categories').submit();
		}else
		{
			alert('<?php echo lang('custom_categories_checked_checkbox_msg');?>');
   ...
   jQuery('#btn-delete').click(function(){
		if(jQuery('input.checkb').is(':checked'))
		{
			cf = confirm('<?php echo lang('custom_categories_confirm_delete_msg');?>');
			if(cf)
			{
				jQuery('#fr-categories').attr('action', '<?php echo site_url().'admin/custom/delcategory'?>');
				jQuery('#fr-categories').submit();
			}
		}else
		{
		...
		if($.isNumeric(value) && (value % 1 == 0) && value >= 0)
		{
			jQuery.ajax({
				...
				success: function(data){
					if(data != '')
					{
						alert('<?php echo lang('custom_categories_save_order_success_msg');?>');
					}else
					{
						alert('<?php echo lang('custom_categories_save_order_error_msg');?>');
					}
				}
			});
		}
