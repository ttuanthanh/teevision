	...
	function getPhoto($photos){
		if(is_array($photos)){
			...
			foreach ($photos as $photo) {
				$data['photo'][] = "https://farm".$photo['farm'].".staticflickr.com/".$photo['server']."/".$photo['id']."_".$photo['secret']."_m.jpg";
				$data['photo_big'][] = "https://farm".$photo['farm'].".staticflickr.com/".$photo['server']."/".$photo['id']."_".$photo['secret']."_b.jpg";
	...
	function getALLAlbum(){
		if($_SESSION['user_flickr_id'] != ''){
			...
			foreach($photos['photoset'] as $photo){
				$data['cover_photo'][] = "https://farm".$photo['farm'].".staticflickr.com/".$photo['server']."/".$photo['primary']."_".$photo['secret']."_m.jpg";
				$data['title'][] = $photo['title']['_content'];
				$data['description'][] = $photo['description']['_content'];
				$data['album_id'][] = $photo['id'];
				$data['count'][] = $photo['photos'];
			}
			return $data;
		}
	}
