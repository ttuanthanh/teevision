...
<form name="setting" class="setting-save" method="POST" action="<?php echo site_url('menu/admin/setting/save/'.$id); ?>">
<div class="tabpanel" role="tabpanel">
	...
	<ul class="nav nav-tabs" role="tablist">
		...
		<li class="button-back pull-right"><a href="javascript:void(0)" onclick="grid.module.setting('menu')" title="Back to list"><i class="clip-arrow-left-2"></i></a></li>    
	</ul>
	...
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane active" id="tab-general">	
			<div class="form-group">
				...
				<input type="text" class="form-control input-sm validate required" name="title" placeholder="<?php echo lang('title');?>" value="<?php echo $menu->title; ?>" data-minlength="2" data-maxlength="200" data-msg="<?php echo lang('menu_admin_setting_title_validate');?>"> 
			</div>
			...
			<div class="form-group">
				<div class="row">
					<div class="col-sm-6">
						<label><?php echo lang('menu_type_title');?></label>
						<select class="form-control input-sm" name="options[menu_type]">
						<?php 
						if (empty($options->menu_type)) $options->menu_type = 0;
						for($i=0; $i<count($menu_type); $i++)
						{
							if ($options->menu_type == $menu_type[$i]->id)
								$select = 'selected="selected"';
							else
								$select = '';
							echo '<option '.$select.' value="'.$menu_type[$i]->id.'">'.$menu_type[$i]->title.'</option>';
						}
						?>
						</select>
					</div>
					...
					<div class="col-sm-6">
						<label><?php echo lang('menu_admin_setting_class_sfx_title');?></label>
						<input type="text" name="options[class_sfx]" class="form-control input-sm" value="<?php if(isset($options->class_sfx)) echo $options->class_sfx; ?>" placeholder="<?php echo lang('menu_admin_setting_class_sfx_title');?>"/>
					</div>
				</div>
			</div>
			...
			<div class="form-group">
				<div class="row">
					<div class="col-sm-6">
						<label><?php echo lang('menu_admin_setting_nav_style_title');?></label>
						<?php
							$style = array(
								'navbar-default'=>lang('menu_admin_setting_nav_default_title'),
								'navbar-inverse'=>lang('menu_admin_setting_nav_inverse_title'),
							);
							if(isset($options->style))
								$default = $options->style;
							else
								$default = '';
							echo form_dropdown('options[style]', $style, $default, 'class="form-control input-sm"');
						?>
					</div>
					
					<div class="col-sm-6">
						<label><?php echo lang('menu_admin_setting_nav_display_title');?></label>
						<?php
							$display = array(
								'default'=>lang('menu_admin_setting_nav_default_title'),
								'navbar-fixed-top'=>lang('menu_admin_setting_nav_fixed_top_title'),
								'navbar-fixed-bottom'=>lang('menu_admin_setting_nav_fixed_bottom_title'),
								'navbar-static-top'=>lang('menu_admin_setting_nav_static_top_title'),
							);
						...
						<div class="form-group">	
							<?php 
								...
								if($image != '')
								{
									...
									echo '<img src="'.$image.'" class="pull-left box-image" style="width: 80px;" alt="" width="100" />';
								}else
								{
