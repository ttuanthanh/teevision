...
if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	if(count($articles))
	{
		...
		if(isset($options->class_sfx) && $options->class_sfx != '')
			echo '<div class="module-lastest_article '.$options->class_sfx.'">';
		else
			echo '<div class="module-lastest_article">';
		
		//show title.
		if(isset($options->show_title) && $options->show_title == 'yes')
			echo '<h4>'.$lastest_article->title.'</h4>';
		
		foreach($articles as $article)
		{
			echo '<div class="lastest_article">';
			echo '<div class="row">';
			// show thumbnail.
