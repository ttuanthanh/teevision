...
<form name="setting" class="setting-save" method="POST" action="<?php echo site_url('row/admin/setting/save/'.$id); ?>">
<div class="tabpanel" role="tabpanel">
	...
	<ul class="nav nav-tabs" role="tablist">
		...
		<li role="presentation"><a href="#tab-design" aria-controls="tab-design" role="tab" data-toggle="tab">Design Options</a></li>    
	</ul>
			...
			<div class="row col-md-12">
				<div class="form-group pick-color">						
					<div class="input-group">							
						<input type="text" class="form-control color input-sm" name="params[fontColor]" value="<?php echo setParams($params, 'fontColor'); ?>">
						<div class="input-group-addon pick-color-btn">Select Color</div>
					</div>
					<a href="javascript:void(0)" class="btn btn-default btn-sm pick-color-clear">Clear</a>
				</div>
			</div>
			
			<label class="control-label">Extra class name</label>
			<div class="row col-md-12">
				<div class="form-group">
					<input type="text" class="form-control" name="options[class_sfx]" value="<?php echo setParams($options, 'class_sfx'); ?>">
					<p class="help-block"><small>If you wish to style particular content element differently, then use this field to add a class name and then refer to it in your css file.</small></p>
				</div>
			</div>
		</div>
		...
		<div role="tabpanel" class="tab-pane" id="tab-design">
			<div class="design-box">
				...
				<div class="design-box-right">
						...
						<div class="form-group">
							...
							<select class="form-control input-sm" name="params[borderStyle]">
								<?php $border_style = setParams($params, 'borderStyle');
									for($i=0; $i<12; $i++){ $value = strtolower($options[$i]);
										if($border_style == $value)
											$check = 'selected="selected"';
										else
											$check = '';
									?>
									<option value="<?php echo $value; ?>" <?php echo $check;?>><?php echo $options[$i]; ?></option>
								<?php } ?>
					...
					<div class="row col-md-12">
						<div class="form-group">
							<?php $options = array('repeat'=>'Repeat', 'repeat-x'=>'Repeat x', 'repeat-y'=>'Repeat Y', 'no-repeat'=>'No repeat'); ?>
