...
class Setting extends Admin_Controller
{
	...
	{ 
        ...
        $config['base_url'] = base_url('image/admin/setting/index'); 
        $config['total_rows'] = $this->image_m->getImages(true);
		$config['per_page'] = 10;
		...
		$this->data['images'] = $this->image_m->getImages(false, $config['per_page'], $segment);
	...
	{	
		if($params = $this->input->post('params'))
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				$data['content'] = $this->input->post('image');
				$data['options'] = json_encode($this->input->post('options'));
				$data['params'] = json_encode($params);
				if($id == '')
				{
					$data['type'] = 'image';
					$data['key'] = 'image'.uniqid();
					if($id = $this->image_m->save($data))
					{
						$content = array(
							'id'		=> $id,
							'error'		=> 0,
							...
							'control'	=> 'image',
							'method'	=> 'index',
							'content'	=> '{module:image/index,'.$id.'}'
						);
					}else
					{
						$content = array(
							'error' => 1,
							'content' => lang('image_admin_setting_add_error_msg'),
						);
					}
				}else
				{
					...
					if(count($image) > 0)
					{
						...
						}else
						{
							$content = array(
								'error' => 1,
								'content' => lang('image_admin_setting_edit_error_msg'),
							);
						}
					}else
					{
						$content = array(
							'error' => 1,
							'content' => lang('image_admin_setting_edit_not_found_error_msg'),
						);
					}
				}
			}else
