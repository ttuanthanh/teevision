...
class Setting extends Admin_Controller
{
	...
	{
		parent::__construct();		
		$this->lang->load('c_categories');
		$this->load->model('c_categories_m');
	}
	...
	public function index()
	{
        ...
        $config['total_rows'] = $this->c_categories_m->getCategories(true);
		$config['per_page'] = 10;
		...
		$this->data['categoriess'] = $this->c_categories_m->getCategories(false, $config['per_page'], $segment);
	...
	{ 
		$this->data['id'] = $id;
		if($id == '')
			$categories = $this->c_categories_m->getNew();
		else
			$categories = $this->c_categories_m->getCategory($id);
	...
	public function save($id = '')
	{
		if($params = $this->input->post('params'))
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				if($id == '')
				{
					$data['type'] = 'c_categories';
					$data['key'] = 'categories'.uniqid();
					if($id = $this->c_categories_m->save($data))
					{
						$content = array(
							'id'		=> $id,
							'error'		=> 0,
							...
							'title'		=> $data['title'],
							'module'	=> 'c_categories',
							'control'	=> 'c_categories',
							'method'	=> 'index',
							'content'	=> '{module:c_categories/index,'.$id.'}'
						);
					}else
					{
						$content = array(
				...
				}else
				{
					...
					if(count($categories) > 0)
					{
						if($this->c_categories_m->save($data, $id))
						{
							$content = array(
								'id'		=> $id,
								'error'		=> 0,
