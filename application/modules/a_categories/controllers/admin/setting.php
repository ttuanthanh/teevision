...
class Setting extends Admin_Controller
{
	...
	{
		parent::__construct();		
		$this->lang->load('a_categories');
		$this->load->model('a_categories_m');
	}
	...
	public function index()
	{
        ...
        $config['total_rows'] = $this->a_categories_m->getCategories(true);
		$config['per_page'] = 10;
		...
		$this->data['categoriess'] = $this->a_categories_m->getCategories(false, $config['per_page'], $segment);
	...
	{ 
		$this->data['id'] = $id;
		if($id == '')
			$categories = $this->a_categories_m->getNew();
		else
			$categories = $this->a_categories_m->getCategory($id);
	...
	public function save($id = '')
	{
		if($params = $this->input->post('params'))
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				if($id == '')
				{
					$data['type'] = 'a_categories';
					$data['key'] = 'categories'.uniqid();
					if($id = $this->a_categories_m->save($data))
					{
						$content = array(
							'id'		=> $id,
							'error'		=> 0,
							...
							'title'		=> $data['title'],
							'module'	=> 'a_categories',
							'control'	=> 'a_categories',
							'method'	=> 'index',
							'content'	=> '{module:a_categories/index,'.$id.'}'
						);
					}else
					{
						$content = array(
				...
				}else
				{
					...
					if(count($categories) > 0)
					{
						if($this->a_categories_m->save($data, $id))
						{
							$content = array(
								'id'		=> $id,
								'error'		=> 0,
