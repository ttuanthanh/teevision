		...
		<tbody>
			...
			<?php 
				if(count($customs) > 0)
				{
					...
					<tr>
						<td>
							<a title="Click to choose this module" href="javascript:void(0)" onclick="grid.module.insert('custom', 'index', <?php echo $custom->id; ?>, '<?php echo $custom->key; ?>', '<?php echo $custom->title; ?>')">
								<?php echo $custom->title; ?>
							</a>
						</td>
						<td><?php echo $custom->key; ?></td>
						<td><?php echo $custom->id; ?></td>
						<td class="center"><i class="fa fa-pencil-square-o" onclick="grid.module.setting('custom', <?php echo $custom->id; ?>)" style="cursor: pointer;"></i><span class="page_ajax"><a href="<?php echo site_url().'social/admin/setting/remove/'.$social->id;?>"><i class="glyphicon glyphicon-remove" style="margin-left: 5px; color: #D9534F;"></i></span></td>
					</tr>
				<?php } ?>
			<?php } ?>
